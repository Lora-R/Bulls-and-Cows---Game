# https://levelup.gitconnected.com/learn-python-by-building-a-gui-guessing-game-with-tkinter-9f82291db
from tkinter import *
import random
...
def not_successfully_finish_game(number_to_guess):
    ...
    pop.geometry(f"{w}x{h}+{int(x)}+{int(y)}")
    pop.config(bg="gray")
    pop_label = Label(pop, text=f"Your tries are up\n"
...
if __name__ == '__main__':
    ...
    tk.pack()
    image = PhotoImage(file="C:\\Users\\Me\\Desktop\\images_for_games\\Start.png")

    start_button = Button(tk, image=image,  height=320, width=320, command=main_logic)
    start_button.pack(pady=20)
    # explanation label for the game
    explanation = Label(tk, text="Welcome to the game\n"
